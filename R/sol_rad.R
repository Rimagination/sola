...
#'
#' @description
#' The \code{sol_rad} function estimates daily solar radiation on a horizontal surface using the Angstrom-Prescott model, based on sunshine duration, latitude, and date.
#'
...
#' @param ext_rad Optional numeric vector. Extraterrestrial radiation in MJ/m²/day.
...
sol_rad <- function(lat, date, ssd, N = NULL, ext_rad = NULL, A = 0.25, B = 0.50) {
  ...
  calc_rad <- function(A, B, ssd, N, ext_rad) {
    ...
    result[na_mask] <- NA  # Ensure that if lat or ssd is NA, result is also NA
    return(result)
  }
